  ...
  if (error) return (
    <div className="p-8 text-center text-red-600 max-w-md mx-auto my-12 bg-red-50 border border-red-200 rounded-2xl">
      ...
      <button 
        ...
        className="mt-4 inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-bold hover:bg-red-700 cursor-pointer"
      >
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 bg-white p-6 rounded-2xl shadow-xs border border-gray-100">
        ...
        <div className="flex items-center space-x-3">
          <button
            onClick={fetchDashboardData}
            disabled={loading}
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white rounded-xl text-xs font-black transition-all shadow-xs cursor-pointer"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
        ...
        <div className="bg-white rounded-2xl shadow-xs border border-gray-100 p-5 flex items-center hover:shadow-md transition-shadow">
          ...
          <div>
            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">{t('high_risk_farmers') || "High Risk Distress Farmers"}</p>
          ...
          {data.high_risk_farmers.length === 0 ? (
            ...
            data.high_risk_farmers.map(farmer => (
              <div key={farmer.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50/80 transition-colors">
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="font-extrabold text-gray-900 text-base">{farmer.name}</h3>
                  </div>
                  <div className="flex items-center space-x-4 text-xs text-gray-500 mt-1 font-medium flex-wrap gap-y-1">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="relative max-w-xs w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50"
            />
          </div>
        </div>
