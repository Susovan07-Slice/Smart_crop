...
const Dashboard = () => {
  ...
  if (loading) return (
    <div className="p-8 text-center text-gray-500 flex flex-col items-center justify-center min-h-[50vh]">
      ...
      <p className="text-lg font-bold text-gray-700">Loading {officer{t('district')}} {t('district')} Data...</p>
    </div>
  );
  ...
  if (error) return (
    <div className="p-8 text-center text-red-600 max-w-md mx-auto my-12 bg-red-50 border border-red-200 rounded-2xl">
      ...
      <button 
        ...
        className="mt-4 inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-xl text-base font-bold hover:bg-red-700 cursor-pointer"
      >
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 bg-white p-6 rounded-2xl shadow-xs border border-gray-100">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="bg-blue-100 text-blue-800 text-base font-bold px-2.5 py-0.5 rounded-full border border-blue-200">
              {t('officer_portal')}
            </span>
          </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            onClick={fetchDashboardData}
            disabled={loading}
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white rounded-xl text-base font-black transition-all shadow-xs cursor-pointer"
          ...
          <div className="flex items-center space-x-2 bg-blue-50 border border-blue-200 text-blue-900 px-4 py-2.5 rounded-xl shadow-2xs">
            ...
            <div>
              <span className="text-sm font-black text-blue-600 uppercase block tracking-wider">{t('assigned_jurisdiction')}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
        <div className="bg-white rounded-2xl shadow-xs border border-gray-100 p-5 flex items-center hover:shadow-md transition-shadow">
          ...
          <div>
            <p className="text-base text-gray-500 font-bold uppercase tracking-wider">{t('total_farmers') || "Total {t('district')} Farmers"}</p>
            <p className="text-3xl sm:text-3xl font-black text-gray-900">{data.total_farmers}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xs border border-gray-100 p-5 flex items-center hover:shadow-md transition-shadow">
          ...
          <div>
            <p className="text-base text-gray-500 font-bold uppercase tracking-wider">{t('high_risk_farmers') || "High Risk Distress Farmers"}</p>
            <p className="text-3xl sm:text-3xl font-black text-red-600">{data.high_risk_count}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xs border border-gray-100 p-5 flex items-center hover:shadow-md transition-shadow sm:col-span-2 lg:col-span-1">
          ...
          <div>
            <p className="text-base text-gray-500 font-bold uppercase tracking-wider">{t('district_monitoring_status')}</p>
            <div className="flex items-center space-x-2 mt-0.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
              <p className="text-2xl font-extrabold text-gray-900">{t('active_syncing')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white shadow-xs border border-red-100 rounded-2xl overflow-hidden mb-8">
        <div className="px-6 py-4 border-b border-red-100 bg-red-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h2 className="text-xl font-black text-red-950 flex items-center">
              <AlertTriangle className="w-5 h-5 mr-2 text-red-600" />
              High Risk Interventions ({officer{t('district')}} {t('district')})
            </h2>
            <p className="text-base text-red-700 font-medium mt-0.5">{t('urgent_distress_intervention')}</p>
          </div>
          {data.high_risk_farmers.length > 0 && (
            <span className="text-base font-extrabold px-3 py-1 bg-red-600 text-white rounded-full">
          ...
          {data.high_risk_farmers.length === 0 ? (
            ...
            data.high_risk_farmers.map(farmer => (
              <div key={farmer.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50/80 transition-colors">
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="font-extrabold text-gray-900 text-xl">{farmer.name}</h3>
                  </div>
                  <div className="flex items-center space-x-4 text-base text-gray-500 mt-1 font-medium flex-wrap gap-y-1">
                ...
                <div className="flex items-center space-x-3 self-end sm:self-center">
                  <div className="text-right mr-2">
                    <span className="text-base font-bold text-red-600 bg-red-50 border border-red-200 px-2.5 py-1 rounded-lg">
                  ...
                  {(() => {
                    ...
                    if (isSending) {
                      return (
                        <button disabled className="px-4 py-2 rounded-xl text-base font-black bg-blue-100 text-blue-800 flex items-center border border-blue-200 shadow-2xs opacity-80 cursor-wait animate-pulse">
                    ...
                    if (isSent) {
                      return (
                        <div className="px-4 py-2 rounded-xl text-base font-black bg-emerald-100 text-emerald-800 flex items-center border border-emerald-300 shadow-2xs animate-in zoom-in-95 duration-200">
                    ...
                    return (
                      <button
                        ...
                        className="px-4 py-2 rounded-xl text-base font-black bg-red-600 hover:bg-red-700 text-white active:scale-95 transition-all shadow-xs cursor-pointer flex items-center"
                      >
      ...
      <div className="bg-white shadow-xs border border-gray-100 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-base text-gray-500 font-medium mt-0.5">Comprehensive database of all registered farmers in {officer{t('district')}}</p>
          </div>
          ...
          <div className="relative max-w-xs w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-base font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-base font-black text-gray-500 uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-gray-100 text-base font-semibold text-gray-700">
                  ...
                  return (
                    <tr key={farmer.id} className="hover:bg-blue-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-6">
                        <span className={`px-2 py-0.5 rounded-full text-base font-black ${
                      ...
                      <td className="py-3.5 px-6 text-right">
                        {isSending ? (
                          <button
                            disabled
                            className="inline-flex items-center text-base font-bold text-blue-700 bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-xl shadow-2xs opacity-80 cursor-wait animate-pulse"
                          >
                            <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin text-blue-600" />
                            <span>{t('sending')}</span>
                          </button>
                        ) : isSent ? (
                          <div className="inline-flex items-center text-base font-black text-emerald-800 bg-emerald-100 border border-emerald-300 px-3.5 py-1.5 rounded-xl shadow-2xs animate-in zoom-in-95 duration-200">
                            <CheckCircle2 className="w-4 h-4 mr-1.5 text-emerald-600 shrink-0" />
                            <span>{t('message_sent')}</span>
                          </div>
                        ) : (
                          <button
                            ...
                            className="inline-flex items-center text-base font-black text-blue-600 hover:text-white bg-blue-50 hover:bg-blue-600 border border-blue-300 px-3.5 py-1.5 rounded-xl transition-all shadow-2xs hover:scale-105 active:scale-95 cursor-pointer"
                          >
