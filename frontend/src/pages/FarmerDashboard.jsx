          ...
          {pointsWithY.map((pt, idx) => (
            <g key={idx} className="group cursor-pointer">
              ...
              <text 
                ...
                y={pt.y - 12} 
                textAnchor="middle" 
                className={`text-sm font-black tracking-tight ${isDarkMode ? 'fill-emerald-300' : 'fill-emerald-800'}`}
              >
              ...
              <text 
                ...
                y="145" 
                textAnchor="middle" 
                className={`text-xs font-extrabold uppercase tracking-wider ${isDarkMode ? 'fill-slate-400' : 'fill-slate-600'}`}
              >
                {pt.label}
              </text>
            </g>
      ...
      <div className={`w-full rounded-3xl shadow-sm border p-4 sm:p-7 flex flex-col space-y-5 transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-gray-200 text-gray-900'}`}>
          ...
          <div className="flex items-center space-x-3.5">
            ...
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                {farmerName && (
                  <span className="bg-emerald-700 text-white font-extrabold text-sm px-3 py-1.5 rounded-full flex items-center shadow-2xs">
                ...
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-2xl text-sm sm:text-base font-black shadow-md hover:shadow-lg transition-all active:scale-95 border cursor-pointer ${
                    isDarkMode
                ...
                <button
                  ...
                  className={`relative flex items-center space-x-2 px-4 py-2 rounded-2xl text-sm sm:text-base font-black shadow-md hover:shadow-lg transition-all active:scale-95 border cursor-pointer ${
                ...
                >
                  ...
                  {officerAlerts.filter(a => !a.is_read).length > 0 && (
                    <span className="ml-1.5 bg-yellow-400 text-red-950 text-sm font-black px-2 py-0.5 rounded-full border border-white">
                      {officerAlerts.filter(a => !a.is_read).length} New
                    </span>
                  )}
                </button>
              </div>
              <p className={`text-sm sm:text-base font-semibold mt-1 ${isDarkMode ? 'text-slate-300' : 'text-gray-600'}`}>
        ...
        {officerAlerts.some(a => !a.is_read) && (
          <div className="p-4 bg-gradient-to-r from-red-50 to-amber-50 border-2 border-red-300 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm animate-in fade-in">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h4 className="text-sm font-black text-red-950 uppercase tracking-wider flex items-center gap-1.5">
                  <span>📩 Official Alert from {location} District Officer</span>
                  <span className="bg-red-600 text-white text-xs px-2 py-0.5 rounded-full font-bold">New Message</span>
                </h4>
                <p className="text-sm sm:text-base font-bold text-gray-800 mt-0.5">{officerAlerts.find(a => !a.is_read)?.message}</p>
              </div>
            </div>
            <button 
              onClick={() => {
              ...
              className="px-4 py-2 bg-red-600 hover:bg-red-700 active:scale-95 text-white rounded-xl text-sm font-black shrink-0 shadow-xs cursor-pointer transition-all"
            >
              Read Messages
            </button>
          </div>
        ...
        <div className={`p-5 sm:p-6 rounded-3xl border shadow-md space-y-4 transition-colors duration-300 ${
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b pb-3 border-emerald-200/60">
            <div>
              ...
              <p className={`text-sm font-semibold mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>
            ...
            <div className="flex flex-wrap items-center gap-1.5 bg-emerald-100/70 p-1.5 rounded-2xl border border-emerald-300 w-full sm:w-auto">
              <span className="text-xs font-black uppercase text-emerald-950 px-1.5 flex items-center">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-black transition-all cursor-pointer ${
          ...
          <div className={`p-4 sm:p-5 rounded-2xl border space-y-3 animate-in fade-in duration-200 ${
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b pb-2.5 border-emerald-100 gap-2">
              <span className="text-sm font-black text-emerald-700 uppercase flex items-center gap-1.5">
              ...
              {nlpResponse && !nlpLoading && (
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    className={`text-white text-sm font-black px-3.5 py-1.5 rounded-xl shadow-2xs flex items-center space-x-1.5 transition-all cursor-pointer border ${
                      isSpeaking
            ...
            {nlpLoading ? (
              ...
              <div className="text-sm sm:text-base font-semibold leading-relaxed whitespace-pre-wrap tracking-wide">
                {nlpResponse}
              </div>
            )}
          </div>
        ...
        <div className={`p-4.5 rounded-2xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-800/80 border-slate-700 text-slate-100' : 'bg-gray-50/90 border-gray-200 text-gray-900'}`}>
          <div className={`flex flex-col sm:flex-row sm:items-center justify-between p-3.5 mb-4 rounded-xl border ${isDarkMode ? 'bg-slate-900 border-slate-600' : 'bg-white border-gray-200'} shadow-sm`}>
            <div className="flex items-center text-base font-bold mb-2 sm:mb-0 flex-wrap gap-2">
              <div className="flex items-center">
                <MapPin className="h-4 w-4 mr-1.5 text-red-500 shrink-0" />
                <span className={`uppercase tracking-wide text-sm ${isDarkMode ? 'text-slate-300' : 'text-gray-500'}`}>Farm Location:</span>
              </div>
              <select
                value={location}
                onChange={(e) => handleSelectDistrict(e.target.value)}
                className={`text-base font-black px-3.5 py-1.5 rounded-xl border-2 transition-all outline-none cursor-pointer shadow-2xs ${
                  isDarkMode 
            ...
            <button 
              ...
              className="text-sm font-black uppercase tracking-wider bg-emerald-100 text-emerald-900 hover:bg-emerald-200 px-3 py-1.5 rounded-lg transition-colors border border-emerald-300 cursor-pointer shrink-0"
            >
              🗺️ Map Picker
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className={`flex items-center text-sm font-bold uppercase mb-1.5 ${isDarkMode ? 'text-slate-300' : 'text-gray-700'}`}>
              ...
              <select 
                ...
                className={`w-full border rounded-xl px-3.5 py-2.5 text-base font-semibold focus:ring-2 focus:ring-green-500 outline-none shadow-2xs ${isDarkMode ? 'bg-slate-700 text-white border-slate-600' : 'bg-white text-gray-800 border-gray-300'}`}
              >
            ...
            {/* Land Area Input */}
            <div>
              <div className={`flex justify-between items-center text-sm font-bold uppercase mb-1.5 ${isDarkMode ? 'text-slate-300' : 'text-gray-700'}`}>
                  ...
                  <select
                    ...
                    className={`ml-2 text-xs bg-transparent font-extrabold cursor-pointer outline-none ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}
                  >
        ...
        <div className={`border rounded-2xl p-4 transition-colors ${isDarkMode ? 'bg-emerald-950/40 border-emerald-800 text-slate-100' : 'bg-emerald-50/80 border-emerald-200/90 text-emerald-900'}`}>
          ...
          <div className="grid grid-cols-4 gap-3.5 text-center">
            <div className={`p-3 rounded-xl border shadow-2xs ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-emerald-100'}`}>
              <span className="block text-xs font-bold text-gray-400 uppercase">{t('nitrogen')}</span>
              <span className="text-lg font-black text-emerald-500">{soilProfile.N} <xs className="text-xs">kg/ha</xs></span>
            </div>
            <div className={`p-3 rounded-xl border shadow-2xs ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-emerald-100'}`}>
              <span className="block text-xs font-bold text-gray-400 uppercase">{t('phosphorus')}</span>
              <span className="text-lg font-black text-emerald-500">{soilProfile.P} <xs className="text-xs">kg/ha</xs></span>
            </div>
            <div className={`p-3 rounded-xl border shadow-2xs ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-emerald-100'}`}>
              <span className="block text-xs font-bold text-gray-400 uppercase">{t('potassium')}</span>
              <span className="text-lg font-black text-emerald-500">{soilProfile.K} <xs className="text-xs">kg/ha</xs></span>
            </div>
            <div className={`p-3 rounded-lg border shadow-2xs ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-emerald-100'}`}>
              <span className="block text-xs font-bold text-gray-400 uppercase">{t('soilph')}</span>
        ...
        {loading ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-3">
            ...
            <p className="text-base font-bold text-gray-500">{t('analyzingfarmdata')}</p>
          </div>
        ) : analysisData ? (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className={`p-6 sm:p-7 rounded-3xl border shadow-md relative overflow-hidden transition-colors ${isDarkMode ? 'bg-slate-800/90 border-emerald-500/40 text-slate-100' : 'bg-gradient-to-br from-emerald-50 via-white to-green-50/40 border-emerald-200'}`}>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-emerald-200/60 pb-5">
                <div>
                  <div className="flex items-center space-x-2">
                    <span className="bg-emerald-600 text-white font-extrabold text-sm px-3 py-1 rounded-full uppercase tracking-wider shadow-2xs">
                      {t('top_recommended_crop')}
                    </span>
                    <span className="bg-emerald-100 text-emerald-800 font-bold text-sm px-2.5 py-1 rounded-full">
                      {location} • {season}
                    </span>
                  </div>
                ...
                <div className={`text-left sm:text-right p-3.5 rounded-2xl border shadow-2xs ${
                  ...
                  <span className={`block text-xs font-bold uppercase ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{t('currentmandiprice')}</span>
                  <span className={`text-2xl font-black ${isDarkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>₹{basePrice.toLocaleString('en-IN')} <xs className={`text-sm font-semibold ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>/qtl</xs></span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 mt-5">
                <div className={`p-3.5 rounded-2xl border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-emerald-100'}`}>
                  <span className={`block text-xs font-bold uppercase ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{t('expectedyield')}</span>
                  ...
                  <span className={`block text-xs font-medium mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>Total ~{((analysisData.crop_recommendation?.yield_per_ha || 3.65) * areaHa).toFixed(1)} Tons</span>
                </div>
                <div className={`p-3.5 rounded-2xl border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-emerald-100'}`}>
                  <span className={`block text-xs font-bold uppercase ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{t('expected_net_profit')}</span>
                  ...
                  <span className={`block text-xs font-medium mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>{displayArea} {landUnit === 'Acres' ? 'Acres' : 'Ha'} Total Land</span>
                </div>
                <div className={`p-3.5 rounded-2xl border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-emerald-100'}`}>
                  <span className={`block text-xs font-bold uppercase ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{t('cultivation_cost')}</span>
                  <span className={`text-lg font-extrabold ${isDarkMode ? 'text-slate-200' : 'text-gray-700'}`}>{formatIndianCurrency(75000 * areaHa)}</span>
                  <span className={`block text-xs font-medium mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>{landUnit === 'Acres' ? '₹30,351 / acre' : '₹75,000 / ha'}</span>
                </div>
                <div className={`p-3.5 rounded-2xl border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-emerald-100'}`}>
                  <span className={`block text-xs font-bold uppercase ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{t('profit_margin')}</span>
                  ...
                  <span className={`block text-xs font-medium mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>Return on Investment</span>
                </div>
              </div>
              ...
              {analysisData.crop_recommendation?.reasons && (
                <div className={`mt-5 pt-4 border-t ${isDarkMode ? 'border-slate-700' : 'border-emerald-200/50'}`}>
                  <h4 className={`text-sm font-bold uppercase tracking-wider mb-2 ${isDarkMode ? 'text-emerald-300' : 'text-emerald-900'}`}>💡 {t('why_this_crop_was_recommended')}</h4>
                  <ul className={`grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm font-medium ${isDarkMode ? 'text-slate-200' : 'text-gray-700'}`}>
            ...
            {candidateCrops.length > 0 && (
              <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
                <h3 className={`text-base font-extrabold uppercase tracking-wider mb-3.5 flex items-center ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse text-sm">
                    <thead>
                      <tr className={`border-b text-xs uppercase font-bold ${isDarkMode ? 'bg-slate-900 border-slate-700 text-slate-300' : 'bg-gray-50 border-gray-200 text-gray-500'}`}>
                          ...
                          <tr key={idx} className={`hover:bg-emerald-50/40 transition-colors ${isRecommended ? (isDarkMode ? 'bg-emerald-950/40 text-emerald-200' : 'bg-emerald-50/60') : ''}`}>
                            ...
                            <td className="p-3">
                              <button
                                ...
                                className="bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold px-3 py-1.5 rounded-xl shadow-2xs hover:shadow-xs transition-all flex items-center space-x-1 cursor-pointer"
                              >
            ...
            <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
              <div className="flex justify-between items-center mb-3">
                <h3 className={`text-base font-extrabold uppercase tracking-wider flex items-center ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                  <LineChart className="h-4 w-4 text-emerald-500 mr-2" />
                  {t('mandi_price_trend_forecast')} - {localizedTopCrop} ({location})
                </h3>
                <span className="text-xs bg-emerald-700 text-white font-extrabold px-3 py-1 rounded-full shadow-2xs">
                  {t('forecast_badge')}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                <div className="p-3.5 rounded-2xl border bg-slate-900 text-slate-100 border-slate-700 shadow-md">
                  <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider">{t('today_mandi_price')}</span>
                ...
                <div className="p-3.5 rounded-2xl border bg-emerald-900/90 text-emerald-100 border-emerald-700 shadow-md">
                  <span className="block text-xs font-bold text-emerald-300 uppercase tracking-wider">{t('next_15_days') || '15 Day Forecast'}</span>
                ...
                <div className="p-3.5 rounded-2xl border bg-emerald-800 text-white border-emerald-600 shadow-lg ring-1 ring-emerald-500/40">
                  <span className="block text-xs font-bold text-emerald-200 uppercase tracking-wider">{t('next_30_days') || '30 Day Forecast'}</span>
                ...
                <div className="p-3.5 rounded-2xl border bg-teal-900/90 text-teal-100 border-teal-700 shadow-md">
                  <span className="block text-xs font-bold text-teal-300 uppercase tracking-wider">{t('next_90_days') || '90 Day Forecast'}</span>
            ...
            <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
              <h3 className={`text-base font-extrabold uppercase tracking-wider mb-3 flex items-center ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4 text-sm font-semibold">
              ...
              <div className={`p-3.5 rounded-xl border flex justify-between items-center ${isDarkMode ? 'bg-slate-900 border-emerald-500/40' : 'bg-white border-emerald-300'}`}>
                ...
                <span className={`${(analysisData.profit_analysis?.roi_percent || 19.1) >= 0 ? "bg-emerald-700" : "bg-red-700"} text-white font-black text-base px-4 py-1.5 rounded-full shadow-xs`}>{(analysisData.profit_analysis?.roi_percent || 19.1) > 0 ? "+" : ""}{analysisData.profit_analysis?.roi_percent || 19.1}% {t("roi")}</span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
          <div className={`w-full max-w-2xl rounded-3xl border shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 ${
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-extrabold text-lg sm:text-xl flex items-center gap-2">
                    ...
                    <span className="text-xs bg-emerald-400/30 text-emerald-100 px-2.5 py-0.5 rounded-full font-bold border border-emerald-300/40">
                      Essential Farmer Guide
                    </span>
                  </h3>
                  <p className="text-sm text-emerald-100">Practical Agronomic Insights & Market Advice</p>
                </div>
              </div>
            ...
            <div className="p-5 sm:p-6 overflow-y-auto space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                <div className={`p-3 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-emerald-50/70 border-emerald-200'}`}>
                  <span className="block text-xs font-bold uppercase text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                ...
                <div className={`p-3 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-blue-50/70 border-blue-200'}`}>
                  <span className="block text-xs font-bold uppercase text-blue-600 dark:text-blue-400 flex items-center gap-1">
                    <Droplets className="h-3 w-3" /> Water Need
                  </span>
                  <span className="text-base font-black text-blue-900 dark:text-blue-300 mt-0.5 block truncate">
                ...
                <div className={`p-3 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-amber-50/70 border-amber-200'}`}>
                  <span className="block text-xs font-bold uppercase text-amber-600 dark:text-amber-400 flex items-center gap-1">
                    <DollarSign className="h-3 w-3" /> Net Profit
                  </span>
                  <span className="text-base font-black text-amber-900 dark:text-amber-300 mt-0.5 block">
                ...
                <div className={`p-3 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-teal-50/70 border-teal-200'}`}>
                  <span className="block text-xs font-bold uppercase text-teal-600 dark:text-teal-400 flex items-center gap-1">
                    <ShieldCheck className="h-3 w-3" /> Safety Score
                  </span>
                  <span className="text-base font-black text-teal-900 dark:text-teal-300 mt-0.5 block">
                ...
                return (
                  <div className="space-y-3 pt-1">
                    ...
                    <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800/80 border-slate-700' : 'bg-emerald-50/50 border-emerald-200/80'}`}>
                      <h4 className={`text-sm font-black uppercase tracking-wider flex items-center gap-1.5 mb-1.5 ${
                        isDarkMode ? 'text-emerald-400' : 'text-emerald-900'
                      }`}>
                        <MapPin className="h-4 w-4 text-emerald-600" /> Soil & Sowing Requirements
                      </h4>
                      <p className={`text-sm font-medium leading-relaxed ${isDarkMode ? 'text-slate-200' : 'text-gray-900'}`}>
                        <strong className={`font-black ${isDarkMode ? 'text-white' : 'text-emerald-950'}`}>Soil Type:</strong> {info.soil}
                      </p>
                      <p className={`text-sm font-medium leading-relaxed mt-1 ${isDarkMode ? 'text-slate-200' : 'text-gray-900'}`}>
                    ...
                    <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-emerald-950/60 border-emerald-800' : 'bg-emerald-100/60 border-emerald-300'}`}>
                      ...
                      <p className={`text-sm font-extrabold leading-relaxed ${isDarkMode ? 'text-emerald-200' : 'text-emerald-950'}`}>
                        {info.market}
                      </p>
                    </div>
            ...
            <div className={`px-6 py-3 border-t flex justify-end ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-gray-50 border-gray-200'}`}>
              <button
                onClick={() => setSelectedInsightCrop(null)}
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm px-5 py-2 rounded-xl shadow-xs transition-all cursor-pointer"
              >
                Got It, Close
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[110] flex items-center justify-center p-4 animate-in fade-in">
          <div className={`max-w-lg w-full rounded-3xl shadow-2xl border p-6 space-y-4 relative ${
            ...
            <div className="flex items-center justify-between border-b pb-3 border-gray-200/60">
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-600 font-bold">📍 {location} District Krushi Office</p>
                </div>
              </div>
              ...
              {officerAlerts.length > 0 ? (
                officerAlerts.map(alert => (
                  <div key={alert.id} className="p-4 bg-blue-50/70 border border-blue-200 rounded-2xl space-y-2">
                    ...
                    <p className="text-sm sm:text-base text-gray-800 font-medium leading-relaxed bg-white p-3 rounded-xl border border-blue-100 shadow-2xs">
                      {alert.message}
                    </p>
                    ...
                    <div className="flex justify-end items-center pt-1">
                      <a 
                        ...
                        className="inline-flex items-center text-sm font-black text-emerald-700 hover:text-emerald-800 bg-emerald-100 px-3 py-1.5 rounded-lg transition-colors"
                      >
                ...
                <div className="p-8 text-center text-gray-500 text-base italic">
            ...
            <div className="pt-2 border-t border-gray-200/60 flex justify-end">
              <button
                onClick={() => setIsAlertModalOpen(false)}
                className="px-5 py-2 bg-gray-900 text-white hover:bg-gray-800 rounded-xl text-sm font-bold transition-all shadow-xs cursor-pointer"
              >
                Close Messages
              </button>
            </div>
