    ...
    <div className={`rounded-3xl p-5 sm:p-6 border w-full flex flex-col md:flex-row items-stretch justify-between gap-6 transition-all ${
      ...
      <div className={`flex-1 flex flex-col justify-between space-y-3 pr-0 md:pr-4 md:border-r ${
        ...
        <div className="flex items-center space-x-3 my-1 flex-wrap gap-2">
          ...
          <span className={`text-base font-bold ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>/ 100</span>

          <span className={`text-sm font-black px-3 py-1 rounded-full border shadow-2xs ${theme.badgeClass}`}>
            {theme.badgeText}
          </span>
        </div>

        <p className={`text-sm font-semibold italic ${isDarkMode ? 'text-slate-300' : 'text-gray-600'}`}>
          "{theme.message}"
        </p>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between space-y-2.5 pl-0 md:pl-2">
        ...
        <div className="flex items-center justify-between text-sm space-x-3">
          ...
          <span className={`text-sm font-extrabold w-20 text-right shrink-0 ${theme.loanBurdenPct >= 70 ? 'text-red-500' : (theme.loanBurdenPct >= 40 ? 'text-amber-500' : 'text-emerald-500')}`}>
        ...
        <div className="flex items-center justify-between text-sm space-x-3">
          ...
          <span className={`text-sm font-extrabold w-20 text-right shrink-0 ${theme.weatherRiskPct >= 70 ? 'text-red-500' : (theme.weatherRiskPct >= 40 ? 'text-amber-500' : 'text-emerald-500')}`}>
        ...
        <div className="flex items-center justify-between text-sm space-x-3">
          ...
          <span className={`text-sm font-extrabold w-20 text-right shrink-0 ${theme.windRiskPct >= 70 ? 'text-red-500' : (theme.windRiskPct >= 40 ? 'text-amber-500' : 'text-emerald-500')}`}>
            {theme.windRiskLevel}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between text-sm space-x-3">
          ...
          <span className={`text-sm font-extrabold w-20 text-right shrink-0 ${theme.cropRiskPct >= 70 ? 'text-red-500' : (theme.cropRiskPct >= 40 ? 'text-amber-500' : 'text-emerald-500')}`}>
            {theme.cropRiskLevel}
          </span>
        </div>
        ...
        <div className="flex justify-end pt-1">
          <button
            onClick={onEditLoan}
            className="flex items-center text-sm font-black text-white bg-emerald-600 hover:bg-emerald-700 border border-emerald-500 px-4 py-2 rounded-xl transition-all shadow-sm hover:scale-105 active:scale-95 cursor-pointer"
          >
